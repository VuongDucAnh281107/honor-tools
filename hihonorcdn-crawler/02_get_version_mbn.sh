...
#
# Usage:
# $ ./get_version_mbn.sh update_hihonorcdn_com_all_400000-500000
...
worker() {
    ...
    for xml in `find "$path" -mindepth 1 -maxdepth 1 -name '*_filelist.xml'`; do
        ...
        mbn_version=$(python3 update_zip_version_mbn.py "http://update.hihonorcdn.com/TDS/data/bl/files/$version/f1/full/$fname")
